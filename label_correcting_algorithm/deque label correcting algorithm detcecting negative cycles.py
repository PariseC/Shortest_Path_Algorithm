...
# @Time    : 2021/7/1 16:59
# @Author  : Praise
# @File    : deque label correcting algorithm detcecting negative cycles.py
# @Software: PyCharm
# obj :实现label correcting算法求解最短路问题,检测网络负环
import csv
import sys
...
def read_network_file(node_file,link_file,network):
    ...
    with open(node_file) as node_f:
        ...
        for row in node_reader:
            ...
            network.number_of_check[int(row[0])]=0
    # read link csv file
    with open(link_file) as f:
        ...
        for row in link_reader:
            ...
            network.number_of_nodes =network.number_of_nodes+2
    network.max_link_cost=max(network.link_cost.values())
...
def find_shortest_path(network):
    ...
    while len(SEList)>0:
        ...
        for to_node_id in network.node_neighbors[current_node_id]:
            ...
            network.number_of_check[to_node_id]+=1
            if new_label<node_label_cost[to_node_id]:
                ...
                # check whether the new label has fallen below -n*max_link_cost(detecting negative cycle)
                if new_label<-network.max_link_cost or network.number_of_check[to_node_id]>network.number_of_nodes-1:
                    path_node_id_list = [to_node_id]
                    pre_node_id = node_predecessor[to_node_id]
                    while pre_node_id != to_node_id:
                        path_node_id_list.insert(0, pre_node_id)
                        pre_node_id = node_predecessor[pre_node_id]
                    path_node_id_list.insert(0, to_node_id)
                    path = '-'.join([str(i) for i in path_node_id_list])
                    print("there exists a negative cycle:%s"%path)
                    sys.exit(0)
